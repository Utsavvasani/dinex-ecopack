...
const labelingTypes = [
  { value: 'Private Labeling', label: 'Private Labeling (OEM)' },
  { value: 'Custom Packaging', label: 'Custom Packaging Design' },
  { value: 'Co-Branding', label: 'Co-Branding' },
  { value: 'Other', label: 'Other' },
];

function LabelingTypeSelector({ value, onChange, error }) {
  const [open, setOpen] = useState(false);
  const ref = useRef(null);
  ...
  const selected = labelingTypes.find(s => s.value === value);
    ...
    <div className="relative" ref={ref}>
      ...
      >
        <span className={selected ? 'text-gray-800' : 'text-gray-400'}>
          {selected ? selected.label : 'Select Service Type'}
        </span>
...
export default function BrandLabelingPage() {
  ...
  const [labelingType, setLabelingType] = useState('');
  ...
  const handleSubmit = async () => {
    ...
    if (!labelingType) newErrors.labelingType = 'Select a type';
    if (!quantity.trim()) newErrors.quantity = 'Estimated quantity required';
    if (!message.trim()) newErrors.message = 'Inquiry details required';
    ...
    try {
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({
          ...
          phone: phone.trim(),
          subject: "Brand Labeling Inquiry",
      ...
      if (data.success) {
        ...
        setPhone(''); setLabelingType(''); setQuantity(''); setMessage('');
      } else {
      ...
      <div className="w-full max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <h1 className="text-4xl md:text-6xl font-black tracking-tight text-gray-900 mb-6">
            Your Brand, Our <span className="text-primary">Quality</span>
          </h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed">
            Elevate your brand with customized, eco-friendly tableware solutions. We provide end-to-end private labeling and custom packaging services.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="space-y-12">
            <div>
              <h2 className="text-3xl font-bold text-gray-900 mb-8">Custom Solutions</h2>
              <div className="grid gap-6">
                {[
                  { icon: Palette, title: "Custom Branding", desc: "Integrate your logo and brand elements seamlessly onto our sustainable tableware products." },
                  { icon: Box, title: "Tailored Packaging", desc: "Create bespoke packaging designs that reflect your brand identity and meet retail requirements." },
                  { icon: Layers, title: "Flexible Quantities", desc: "Benefit from our scalable production capacity, catering to both emerging brands and large retailers." }
                ].map((item, i) => (
            ...
            <div className="p-8 rounded-3xl bg-purple-50/50 border border-purple-100">
              ...
              <p className="text-sm text-gray-600 leading-relaxed">
                Our team ensures that your private-labeled products are retail-ready with proper barcoding, compliance labeling, and premium shelf presentation.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-8 xl:p-12 shadow-sm ring-1 ring-gray-100 min-h-[500px] flex flex-col justify-center">
            {!showSuccess ? (
              <>
                <h2 className="text-2xl font-bold text-gray-900 mb-2">Labeling Inquiry</h2>
                <p className="text-gray-500 text-sm mb-8">Tell us about your brand and your vision for custom sustainable products.</p>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-gray-700">Company / Brand Name <span className="text-red-400">*</span></label>
                    <input type="text" value={companyName} onChange={e => setCompanyName(e.target.value)}
                      className={errors.companyName ? errorInputClass : inputClass} placeholder="Your Brand Name" />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                        className={errors.email ? errorInputClass : inputClass} placeholder="email@brand.com" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-semibold text-gray-700">Est. Monthly Qty <span className="text-red-400">*</span></label>
                      <input type="text" value={quantity} onChange={e => setQuantity(e.target.value)}
                        className={errors.quantity ? errorInputClass : inputClass} placeholder="e.g. 50,000 units" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-gray-700">Service Required <span className="text-red-400">*</span></label>
                    <LabelingTypeSelector value={labelingType} onChange={setLabelingType} error={errors.labelingType} />
                  </div>

                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-gray-700">Your Requirements <span className="text-red-400">*</span></label>
                    <textarea rows={4} value={message} onChange={e => setMessage(e.target.value)}
                      className={`resize-y ${errors.message ? errorInputClass : inputClass}`}
                      placeholder="Tell us about the products you're interested in labeling and any specific design ideas..." />
                  </div>
                  ...
                  >
                    {loading ? "Sending Request..." : "Submit Labeling Inquiry"}
              ...
              <div className="text-center animate-in fade-in zoom-in duration-500">
                ...
                <p className="text-gray-500 mb-8 max-w-sm mx-auto leading-relaxed">
                  Your brand labeling inquiry has been received. Our branding specialists will contact you to discuss your requirements in detail.
                </p>
              </div>
            )}
          </div>
